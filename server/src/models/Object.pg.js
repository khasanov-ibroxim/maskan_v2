// server/src/models/Object.pg.js - ✅ FIXED: olx_id column

const { query } = require('../config/database');

class PropertyObject {
    /**
     * ✅ FIXED: Save with olx_id column
     */
        ...
        try {
            ...
            if (existing.rows.length > 0) {
                ...
                const result = await query(
                    `UPDATE objects
                     SET sana = $1, m2 = $2, narx = $3, fio = $4,
                         uy_turi = $5, xolati = $6, planirovka = $7,
                         balkon = $8, torets = $9, dom = $10, kvartira = $11,
                         osmotir = $12, opisaniya = $13, rieltor = $14,
                         xodim = $15, sheet_type = $16, rasmlar = $17,
                         phone_for_ad = $18, updated_at = CURRENT_TIMESTAMP
                     WHERE unique_id = $19
                     RETURNING *`,
                    [
            ...
            // ✅ INSERT with olx_id (NULL initially)
            const result = await query(
                `INSERT INTO objects (
                    ...
                    sheet_type, rasmlar, phone_for_ad, elon_status, olx_id
                )
                 VALUES (
            ...
            console.log('   OLX ID:', result.rows[0].olx_id || 'NULL (not posted yet)');
    ...
    /**
     * ✅ FIXED: Set posted with OLX ID extraction
     */
    static async setPosted(id, adUrl) {
        try {
            // ✅ Extract OLX ID from URL
            // Example URL: https://www.olx.uz/d/ad/kvartira-ID12345678.html
            let olxId = null;
            ...
            if (adUrl) {
                ...
                if (match) {
                    olxId = match[1];
                    console.log('  📝 OLX ID extracted:', olxId);
                } else {
                    console.log('  ⚠️ OLX ID not found in URL');
                }
            }
            ...
            if (olxId) console.log(`   OLX ID: ${olxId}`);
    ...
    static async getStats() {
        const result = await query(`
            SELECT
                ...
                COUNT(*) FILTER (WHERE olx_id IS NOT NULL) as with_olx_id,
    ...
    static async getByKvartil() {
        const result = await query(`
            SELECT
                ...
                COUNT(*) FILTER (WHERE olx_id IS NOT NULL) as with_olx_id_count
            FROM objects
            GROUP BY kvartil
            ORDER BY count DESC
        `);

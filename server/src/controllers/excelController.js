...
/**
 * ✅ FIXED: Delete object WITH folder deletion
...
exports.deleteObject = async (req, res) => {
    try {
        ...
        // 1. Get object data (need rasmlar URL to find folder)
        ...
        if (object.rasmlar && object.rasmlar !== "Yo'q") {
            try {
                ...
                if (urlParts.length > 1) {
                    ...
                    console.log('    Encoded Path:', encodedPath.substring(0, 50) + '...');
                    ...
                    if (fs.existsSync(folderPath)) {
                        ...
                        console.log('    ✅ Folder o\'chirildi');
                        folderDeleted = true;
                    } else {
                        console.log('    ⚠️ Folder topilmadi:', folderPath);
                    }
                } else {
                    console.log('    ⚠️ URL formatida xato');
                }
            } catch (folderError) {
                console.error('    ❌ Folder o\'chirishda xato:', folderError.message);
            }
        } else {
            console.log('\n  ℹ️ Rasmlar URL yo\'q - folder o\'chirilmaydi');
        }

        // 3. Remove from queue if present
        const queueIndex = global.adQueue?.indexOf(id) ?? -1;
        if (queueIndex !== -1) {
            global.adQueue.splice(queueIndex, 1);
            console.log('\n  🗑️ Navbatdan o\'chirildi');
        }

        // 4. Delete from PostgreSQL
        await PropertyObject.delete(id);
        console.log('\n  ✅ PostgreSQL\'dan o\'chirildi');

        console.log('\n' + '='.repeat(60));
        console.log('✅ OBYEKT MUVAFFAQIYATLI O\'CHIRILDI');
        console.log('='.repeat(60) + '\n');
